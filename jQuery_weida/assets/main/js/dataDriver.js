  ...
  var __PROTOTYPE__ = {
    ...
    'driver' : function(){
      for(var model in this){
        if(this[model].load){
          this[model].beforeRender&&this[model].beforeRender(this);
          this[model].render&&this[model].render(this);
          this[model].afterRender&&this[model].afterRender(this);
   ...
   return $.extend({
    ...
    'detail':{
      ...
      'render':function(){
        for(var pro in this.data){
    			var $obj = $('#'+pro);
    			$obj.text(Utils.format($obj, this.data[pro]));
    		}
      }
    },//详细数据模块
    'invest': {
      ...
      "renderPie":function(metaData){
				...
				for(var prop in this.data.pie1){
					...
					pie1Detail.append('<dl><dt>'+prop+'</dt><dd class="number" data-format-type="money">'+this.data.pie1[prop]+'</dd><dd class="unit">元</dd></dl>');
				}
		        option = {
		        	title : {
		        		text:"可投票据概览（万）"
				  ...
				  title : {
        	        ...
        	        feature : {
                      ...
                      myTool : {
                        show : true,
                        title : '返回',
                        icon : 'image://assets/images/back.png',
                        onclick : function (){
                            $(line).hide().prev().show();
                        }
                    }
			...
			"renderBar":function(metaData){
		        ...
		        myChart.on('click', function (params) {
		  		...
		  		for(var prop in this.data.bar){
					...
					barDetail.append('<dl><dt>'+prop+'</dt><dd class="number" data-format-type="money">'+this.data.bar[prop]+'</dd><dd class="unit">元</dd></dl>');
				}
	            var option = {
	        	    title : {
	        	        text: "现金流量（万）",
    ...
    'list':{
      ...
      'beforeRender':function(metaData){
        var data = this.data;
        metaData.query.pageNo = data.pageNo;
        metaData.query.pageSize = data.pageSize;
        metaData.query.pageSize = data.pageSize;
        ...
        var row,$row;
				...
				for(var i=0;i<this.data.result.length;i++){
					...
					$row = $("<tr>"+
								        ...
								        "<td><span data-format-type='money'>"+row.faceAmt+"</span>元</td>"+
								        "<td><span data-format-type='money'>"+row.subscriptionAmt+"</span>元</td>"+
